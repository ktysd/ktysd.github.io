...
///// 運動方程式の定義 (2パターン)/////
function dx = eom1(t,x)  //赤を右に倒す方程式
    pp = rinku(x);    //リンクからの反力
    ///// 左 /////
    a=x(1); da=x(2); b=x(3); db=x(4);
...
function dx = eom2(t,x)  //赤を左に倒す方程式
    pp = rinku(x);    //リンクからの反力
    ///// 左 /////
    a=x(1); da=x(2); b=x(3); db=x(4);
    ff=yuka([a; da; b; db]);            //床反力
...
///// 運動方程式を解く (2パターン))/////
b=int(10*rand()); //0～1の乱数から整数部を取り出す
D=modulo(b,2)     //取り出した整数の偶奇の判断をmodで
                  //少しごまかしてるので正確に1/2の確率ではない...
  if(D==1) //赤を右に倒す
      tn=501; tt=linspace(0,0.01*tn,tn);
      x01=[0; 0; 0; 0];       //左側の初期値
      x02=[w0/r; 0; 0; 0];    //右側の初期値
      xx=ode( [x01;x02], 0, tt, eom1 );
  else    //赤を左に倒す
      tn=701; tt=linspace(0,0.01*tn,tn);
      x01=[0; 0; 0; 0];       //左側の初期値
      x02=[w0/r; 0; 0; 0];    //右側の初期値
      xx=ode( [x01;x02], 0, tt, eom2 );
  end
///// ロボット描画用の関数 /////
function R=Rot(a)           //回転
    R=[cos(a),-sin(a);...
